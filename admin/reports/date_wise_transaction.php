...
<div class="card card-outline card-primary rounded-0 shadow">
	<div class="card-header">
		<h3 class="card-title">تقرير المعاملات حسب التاريخ</h3>
					...
					<form action="" id="filter">
						<div class="row align-items-end">
							<div class="form-group col-md-3">
								<label for="from" class="control-label">من تاريخ</label>
                                <input type="date" name="from" id="from" value="<?= $from ?>" class="form-control form-control-sm rounded-0">
							</div>
							<div class="form-group col-md-3">
								<label for="to" class="control-label">إلى تاريخ</label>
                                <input type="date" name="to" id="to" value="<?= $to ?>" class="form-control form-control-sm rounded-0">
							</div>
							<div class="form-group col-md-4">
                                <button class="btn btn-primary btn-flat btn-sm"><i class="fa fa-filter"></i> تصفية</button>
			                    <button class="btn btn-sm btn-flat btn-success" type="button" id="print"><i class="fa fa-print"></i> طباعة</button>
							</div>
						</div>
					</form>
			</fieldset>
        ...
        <div class="container-fluid">
			<div class="row">
				<div class="col-2 d-flex justify-content-center align-items-center">
					<img src="<?= validate_image($_settings->info('logo')) ?>" class="img-circle" id="sys_logo" alt="شعار النظام">
				</div>
				<div class="col-8">
					<h4 class="text-center"><b><?= $_settings->info('name') ?></b></h4>
					<h3 class="text-center"><b>تقرير معاملات التأمين حسب التاريخ</b></h3>
					<h5 class="text-center"><b>حتى</b></h5>
					<?php if($from == $to): ?>
			...
			<table class="table table-bordered table-hover table-striped">
				...
				<tbody>
					...
					<?php if($qry->num_rows <= 0): ?>
						<tr>
							<th class="py-1 text-center" colspan="8">لا توجد بيانات.</th>
						</tr>
					<?php endif; ?>
				</tbody>
			</table>
...
<script>
	$(document).ready(function(){
       ...
       $('#print').click(function(){
		   ...
		   var _h = $('head').clone()
		   var _el = $('<div>')
		   _h.find("title").text("تقرير المعاملات حسب التاريخ - عرض الطباعة")
