...
Class Master extends DBConnection {
    ...
    function save_bus(){
        ...
        $data = "";
        foreach($_POST as $k => $v){
            if(!in_array($k, array('id','ajax','save_bus'))){
				if(!is_numeric($v))
					$v = $this->conn->real_escape_string($v);
				if(!empty($data)) $data .= ",";
				$data .= " `{$k}`='{$this->conn->real_escape_string($v)}' ";
		...
		if(empty($id)){
			$sql = "INSERT INTO `buses` SET {$data}, `date_created` = CURRENT_TIMESTAMP()";
		}else{
	...
	function delete_bus(){
		extract($_POST);
		$del = $this->conn->query("UPDATE `buses` SET delete_flag = 1 WHERE id = '{$id}'");
		if($del){
			$resp['status'] = 'success';
			$resp['msg'] = 'تم الحذف بنجاح';
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
...
switch ($action) {
	...
	break;
	case 'save_bus':
		echo $Master->save_bus();
	break;
	case 'delete_bus':
		echo $Master->delete_bus();
	break;
	default:
		// echo $sysset->index();
		break;
